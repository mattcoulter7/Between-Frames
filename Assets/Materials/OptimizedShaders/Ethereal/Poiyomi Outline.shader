Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Outline/61d680f6efef7b442973c34d9fb69f95"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			v2f vert(appdata v)
			{
				...
				float3 outlineNormal = float(0) ? o.normal : v.normal;
				if (float(0) == 1)
				{
				...
				if (float(1))
				{
					if (LumaIsAvailable())
					{
						lineWidth += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, LumaData(0.578, 0.515)[float(0) / 2]);
					}
					else if (AudioLinkIsAvailable())
					{
						lineWidth += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, AudioLinkData(uint2(0, float(0))));
					}
				}
				#endif
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				#ifdef POI_PASS_OUTLINE
				shadowStrength = lerp(0, shadowStrength, float(0));
				#endif
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void applyOutlineColor(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					...
					if (poiMods.audioLinkAvailable)
					{
						lineWidth += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]);
					}
					#endif
				...
				if (float(1) == 2)
				{
					poiFragData.baseColor = lerp(poiFragData.baseColor, poiLight.directColor, float(0));
				}
				if (float(0))
				{
				...
				if (poiMods.audioLinkAvailable)
				{
					emission += lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]);
				}
				#endif
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(9) != POI_MODE_TRANSPARENT)
				{
