Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/61d4513b0d2c894488517443d8f175db"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(1));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(1), float(0.07));
				}
				else
				{
					finalWrap = max(finalWrap, float(1));
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(1));
					poiLight.indirectColor = max(poiLight.indirectColor, float(1));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(1))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(1))));
				}
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(0.07));
					poiLight.indirectColor = min(poiLight.indirectColor, float(0.07));
				}
				if (float(0))
				{
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0.1132075,0.1132075,0.1132075,1).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0.1132075,0.1132075,0.1132075,1).a;
