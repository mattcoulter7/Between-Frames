Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/af74fda7e8a227642b95865060a1eec4"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					mainUV = sharpSample(float4(0.0078125,0.00390625,128,256), mainUV);
				}
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -float(-0.5) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(-0.272) * hueShiftAlpha.g);
				#endif
