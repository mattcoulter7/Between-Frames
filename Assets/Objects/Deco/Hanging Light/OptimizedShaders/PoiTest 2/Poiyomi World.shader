Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/206fa85742e6b5d48a2dee5bf11fcfac"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_REALISTIC 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.5));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.5), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.5));
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0))
				{
					mainUV = sharpSample(float4(0.00390625,0.0078125,256,128), mainUV);
				}
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(4.11,4.94,-0.62,0.03)) + _Time.x * float4(0,0,0,0));
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.5));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.5));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.5))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.5))));
				}
