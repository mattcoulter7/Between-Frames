Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/64d2670632e8b9c4c8940142ab81512d"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void applyShadeMapping(inout PoiFragData poiFragData, PoiMesh poiMesh, inout PoiLight poiLight)
			{
				...
				mainShadowMask *= poiLight.shadowMask * float(0.586);
				firstSecondShadowMask *= poiLight.shadowMask * float(0.586);
				if (float(0) == 0)
				{
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				float shadowStrength = float(0.586) * poiLight.shadowMask;
				#ifdef POI_PASS_OUTLINE
				...
				if (float4(0.7,0.75,0.85,1).a > 0)
				{
					...
					#endif
					indirectColor = lerp(float3(1, 1, 1), shadowColorTex.rgb, shadowColorTex.a) * float4(0.7,0.75,0.85,1).rgb;
				}
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.693));
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.427));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				float3 mathRamp = lerp(float3(1, 1, 1), saturate(lerp((float4(1,1,1,1) * lerp(indirectColor, 1, float(0.693))), float3(1, 1, 1), saturate(poiLight.rampedLightMap))), float(0.586));
				float3 finalWrap = directColor + indirectColor;
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.427), float(8.04));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.427));
				}
				poiLight.finalLighting = finalWrap * saturate(mathRamp + 1 - float(0.586));
				#endif
				...
				poiLight.finalLighting = min(lerp(poiLight.indirectColor * float4(1,1,1,1), float4(1,1,1,1), float(0.693)) + (sss * poiLight.directColor), poiLight.directColor);
				#endif
				#ifdef _LIGHTINGMODE_SDF
				...
				indirectColor = indirectColor * lerp(poiLight.indirectColor, poiLight.directColor, float(0.693));
				indirectColor = lerp(poiLight.directColor, indirectColor, float(0.586) * poiLight.shadowMask);
				...
				if (float(1) == 1)
				{
					...
					#endif
					if (float(0) == float(0)) float(0) += 0.001;
					poiLight.rampedLightMap = smoothstep(float(0), float(0), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep(float(0), float(0), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
				}
				if (float(1) == 2)
				{
				...
				for (int index = 0; index < 4; index++)
				{
					...
					if (float(1) == 1) // Toon
					{
						vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * float(0.5) * poiLight.vAttenuation[index], smoothstep(float(0), float(0), .5 * poiLight.vDotNL[index] + .5)) * poiLight.detailShadow;
					}
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float3 mainNormal = UnpackScaleNormal(POI2D_SAMPLER_PAN(_BumpMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)), float(1.39));
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.427));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.427));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.427))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.427))));
				}
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(8.04));
					poiLight.indirectColor = min(poiLight.indirectColor, float(8.04));
				}
				if (float(0))
				{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(3) != POI_MODE_TRANSPARENT)
				{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				meta.Emission = poiFragData.emission * float(1) * poiMods.globalEmission;
