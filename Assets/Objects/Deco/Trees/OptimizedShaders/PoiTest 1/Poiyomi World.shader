Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/2fe1112d801790a4293a349231496e70"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0), float(0));
				}
				else
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(1))
				{
					poiLight.directColor = min(poiLight.directColor, float(0));
					poiLight.indirectColor = min(poiLight.indirectColor, float(0));
				}
				if (float(0))
				{
