Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi World/2ef0a7a3dfed62844aa59f3948a8a882"
{
	Properties
	{
 ...
 #define _LIGHTINGMODE_FLAT 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (!float(0))
				{
					emissionColor0 = POI2D_SAMPLER_PAN(_EmissionMap, _MainTex, poiUV(poiMesh.uv[float(0)], float4(1,1,0,0)), float4(0,0,0,0)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.2966803,0.5025603,0.5566038,1).rgb, float(0));
				}
				else
				{
					emissionColor0 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * float(5)).rgb * lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.2966803,0.5025603,0.5566038,1).rgb, float(0));
				}
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, float(1)).rgb * poiThemeColor(poiMods, float4(0.2966803,0.5025603,0.5566038,1).rgb, float(0));
				#endif
				if (float(0))
				{
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -float(5.7) * hueShiftAlpha.b);
				poiFragData.baseColor = saturate(poiFragData.baseColor + float(1) * hueShiftAlpha.g);
				#endif
