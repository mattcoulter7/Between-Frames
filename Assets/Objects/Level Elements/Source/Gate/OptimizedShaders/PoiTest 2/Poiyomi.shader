Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/af898d1c83083194699ea3d89e7a8c06"
{
	Properties
	{
 ...
 #define PROP_LIGHTINGAOMAPS 
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				poiLight.rampedLightMap = poiLight.nDotLSaturated;
				poiLight.finalLighting = max(UNITY_BRDF_PBS(1, 0, 0, 0, poiMesh.normals[1], poiCam.viewDir, light, CreateIndirectLight(poiMesh, poiCam, poiLight)).xyz, float(0.3));
				#endif
				#ifdef _LIGHTINGMODE_CLOTH
				...
				if (float(1))
				{
					finalWrap = clamp(finalWrap, float(0.3), float(1));
				}
				else
				{
					finalWrap = max(finalWrap, float(0.3));
				}
			...
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				if (float(0) == 3)
				{
					poiLight.directColor = max(poiLight.directColor, float(0.3));
					poiLight.indirectColor = max(poiLight.indirectColor, float(0.3));
				}
				else
				{
					poiLight.directColor = max(poiLight.directColor, poiLight.directColor / max(0.0001, (calculateluminance(poiLight.directColor) / float(0.3))));
					poiLight.indirectColor = max(poiLight.indirectColor, poiLight.indirectColor / max(0.0001, (calculateluminance(poiLight.indirectColor) / float(0.3))));
				}
