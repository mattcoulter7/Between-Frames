...
public class PlayBarSound : MonoBehaviour
{
    ...
    private bool BarMoving; // flag for knowing if we have started sounds
    ...
    private void FixedUpdate()
    {
        ...
        //    StartSound();
        //}
        //if presssed then start the sound

        if (BarMoving)
        {
            // The pitch is interpolated between the min and max values, according to the car's revs.
            float pitch = ULerp(lowPitchMin, lowPitchMax, 1);//m_CarController.engineRPM / m_CarController.maxRPM)
